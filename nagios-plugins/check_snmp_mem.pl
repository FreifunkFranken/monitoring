#!/usr/bin/perl -w 
#
# 2007 ov@odn.de -- mit dem Parameter -d ist nun einstellbar ob UDP (default) oder TCP-Protokoll verwendet werden soll
#
############################## check_snmp_mem ##############
# Version : 0.9
# Date : Jul 20 2005
...
my $o_domain=   'udp';          # udp as default
...
sub print_usage {
    print "Usage: $0 [-v] -H <host> -C <snmp_community> [-2] | (-l login -x passwd)  [-p <port>] -w <warn level> -c <crit level> [-t <udp|tcp] [-I|-N|-E] [-f] [-t <timeout>] [-V]\n";
}
...
-d, --domain=udp | tcp
   use udp (default) or tcp connection
-N, --netsnmp (default)
